...
#Load data
clustering1<-read.csv("https://xiaoruizhu.github.io/Data-Mining-R/data/qry_Food_by_Month.csv")
...
for (i in 2:12) wss[i] <- sum(kmeans(norm_data, centers=i)$withinss)
plot(1:12, wss, type="b", xlab="Number of Clusters",ylab="Within groups sum of squares")
#there is a steep decrease till 3 and after that from 5 to 6
fit3 = kmeans(norm_data, 3)
fit6 = kmeans(norm_data, 6)
plotcluster(norm_data, fit3$cluster)
plotcluster(norm_data, fit6$cluster)

#prediction strength
prediction.strength(norm_data, Gmin=2, Gmax=10, M=10,cutoff=0.8)

#silhoutte method
d = dist(norm_data, method = "euclidean")
result = matrix(nrow = 14, ncol = 3)
for (i in 2:15){
  cluster_result = kmeans(norm_data, i)
...
set.seed(12345)
food.dist=dist(norm_data)

#Obtain clusters using the Wards method
food.hclust=hclust(food.dist, method="ward.D2")
plot(food.hclust)
...
norm_data[food.2clust==1,]
norm_data[food.2clust==2,]
